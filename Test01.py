import ast  # Thư viện hỗ trợ chuyển đổi chuỗi thành list
import os
...
def parse_prize(prize_str_or_list):
    # Kiểm tra nếu dữ liệu đã là danh sách thì trả về ngay
    if isinstance(prize_str_or_list, list):
        return [number.strip('\"') for number in prize_str_or_list]  # Loại bỏ dấu ngoặc kép từ mỗi phần tử trong danh sách
    
    # Chuyển đổi chuỗi thành danh sách bằng cách tách các phần tử bằng dấu phẩy
    prize_list = prize_str_or_list.split(",")
    # Loại bỏ dấu nháy kép và dấu ngoặc vuông từ mỗi phần tử trong danh sách
    prize_list = [number.strip("[]' ") for number in prize_list]
    return prize_list
...
while current_date <= end_date:
    ...
    if not ((current_date >= datetime(2018, 2, 15) and current_date <= datetime(2018, 2, 18)) or
        ...
        if data:
            # Parse special prize and first prize to list format
            data['Giải Đặc BIệt'] = parse_prize(data['Giải Đặc BIệt'])
            data['Giải Nhất'] = parse_prize(data['Giải Nhất'])
